# 哔哩哔哩 · 关注回顾（Tampermonkey 油猴脚本）v1.2.8
...
- **「无投稿」与「采集未成功」严格区分（v1.2.8 起）**：只有「投稿列表为空 **且** 名片接口 `archive_count` 为 0」才判定为「无投稿」；若查询被风控限流（`-352/-412`）或网络超时导致没取到列表，则归入「**采集未成功**」标签页，**保留上次结果**，既不冒充「无投稿」也不会清空已有数据。
...
2. 油猴面板 →「添加新脚本」→ 粘贴 `bilibili subscribe.js` 全文 → `Ctrl+S` 保存；
   - 已经装过旧版的话：打开旧脚本全选替换成新内容保存即可（当前版本 1.2.8，可在脚本头部 `@version` 确认是否更新成功），本地缓存会自动兼容升级；
...
- **「无投稿」里怎么会有明明有稿件的账号？** 这是 v1.2.7 及更早版本的 Bug（**v1.2.8 已修复**）：当时把「投稿查询失败（风控限流 / 网络超时）」也当成了「无投稿」，并且会清掉该 UP 已有的视频数据 —— 所以谁的扫描撞上风控，谁的关注里就会多出几个假的「无投稿」。现在改为用名片接口的稿件数（`archive_count`）交叉验证，升级后第一次「更新数据」会重新核实缓存里所有「无投稿 / 采集未成功」的账号。
